...
public partial class BugEnemy : BotBase
{
	...
	private bool IsPlayerOnSameLevel()
	{
		...
		return Mathf.Abs(_targetPlayer.GlobalPosition.Y - GlobalPosition.Y) <= 80f;
	}

	private bool IsPlayerInAttackRange()
	{
		if (_targetPlayer == null) return false;
		float dx = Mathf.Abs(_targetPlayer.GlobalPosition.X - GlobalPosition.X);
		float dy = Mathf.Abs(_targetPlayer.GlobalPosition.Y - GlobalPosition.Y);
		return dx <= AttackDistance && dy <= AttackDistance;
	}
	...
	private void TryStartAttack()
	{
		...
		SfxManager.Instance?.PlayAt(SfxId.BugSwing, GlobalPosition);
	}
	...
	private void Patrol()
	{
		...
		Velocity = new Vector2(moveDir * MoveSpeed * 0.5f, Velocity.Y);
		OnAnimationRequest?.Invoke("Walk");
	}
	...
	private bool HasGroundAhead(float direction)
	{
		...
		Vector2 from = GlobalPosition + new Vector2(direction * 30f, 10f);
		Vector2 to = from + new Vector2(0f, 60f);
	...
	public void PerformAttack()
	{
		...
		SfxManager.Instance?.PlayAt(SfxId.BugHit, GlobalPosition);
	...
	private void HandleDamage()
	{
		...
		SfxManager.Instance?.PlayAt(SfxId.BugDamage, GlobalPosition);
